  ...
  influencer
    ...
    .then((savedUser) => {
      ...
      if (password === savedUser.password) {
        ...
        const { name, email, phone } = savedUser;
        res.json({ token, user: { name, email, phone }, type: "Influencer" });
      } else {
        return res.status(422).json({ error: "Invalid Email or password" });
      }
    })
  ...
  Shop.findOne({ email: email })
    .then((savedUser) => {
      ...
      if (password === savedUser.password) {
        ...
        const token = jwt.sign({ _id: savedUser._id }, JWT_SECRET);
        const { name, email, phone } = savedUser;
        res.json({ token, user: { name, email, phone }, type: "Shop" });
        return;
      } else {
        return res.status(422).json({ error: "Invalid Email or password" });
      }
    ...
    .then((savedUser) => {
      if (savedUser) {
        if (password === savedUser.password) {
          const token = jwt.sign({ _id: savedUser._id }, JWT_SECRET);
          const { name, email, phone } = savedUser;
          return res.json({ token, user: { name, email, phone }, type: "Admin" });
        } else {
          return res.status(422).json({ error: "Invalid Email or password" });
        }
      } else {
